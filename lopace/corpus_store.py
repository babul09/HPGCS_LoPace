...
class CorpusStore:
    ...
    def __init__(self, db_path: str = ":memory:", zstd_level: int = 15):
    ...
    def store(
        ...
        for seg in segments:
            if seg["type"] == "literal":
                ...
                if full_hash in self._known:
                    self._conn.execute(
                        "UPDATE content_nodes SET ref_count = ref_count + 1 "
                        "WHERE content_hash = ?", (full_hash,),
                    )
                    reused += 1
                else:
        ...
        self._conn.execute(
            ...
            "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            (prompt_id, original_hash, original_size, bp_compressed, bp_size,
             reused + new,
        ...
        return {
            ...
            "marginal_ratio": original_size / marginal if marginal > 0 else float("inf"),
            "marginal_savings_pct": (1 - marginal / original_size) * 100 if original_size else 0.0,
            "compression_time_s": elapsed,
        }

    def store_adaptive(
        self, prompt_id: str, text: str, segments: List[dict],
        cluster_id: str = "", fallback_threshold: float = 0.5,
    ) -> Dict[str, Any]:
        """Adaptive: use dedup when beneficial, monolithic when not."""
        ...
        result["strategy"] = "corpus_dedup" if reuse_fraction >= fallback_threshold else "corpus_dedup_with_new_nodes"
        result["reuse_fraction"] = reuse_fraction
        return result

    def _store_monolithic(self, prompt_id: str, text: str) -> Dict[str, Any]:
        """Fallback: compress entire prompt as one blob."""
        t0 = time.perf_counter()
        ...
        return {
            "prompt_id": prompt_id, "original_size": original_size,
            "blueprint_size": bp_size, "new_bytes_stored": len(compressed),
            "marginal_bytes": marginal, "reused_refs": 0, "new_refs": 1,
            "total_refs": 1,
            ...
            "compression_time_s": elapsed,
            "strategy": "monolithic_fallback", "reuse_fraction": 0.0,
        }
    ...
    def corpus_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "corpus_compression_ratio": total_original / corpus_stored if corpus_stored else 0,
            "corpus_space_savings_pct": (1 - corpus_stored / total_original) * 100 if total_original else 0,
            "node_compression_ratio": total_node_orig / total_node_comp if total_node_comp else 0,
            "storage_overhead_bytes": total_bp,
            "storage_overhead_pct": total_bp / corpus_stored * 100 if corpus_stored else 0,
        }
